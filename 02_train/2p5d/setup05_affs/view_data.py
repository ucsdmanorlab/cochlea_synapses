...
datasets = [i for i in natsorted(os.listdir(sys.argv[1])) if '.' not in i]
...
with viewer.txn() as s:
    ...
    for dataset in datasets:
        ...
        if shape==4:
            n_ch = data.shape[1]
            cent = np.round((n_ch-1)/2).astype('int')
            print(n_ch, cent)
            first = np.max((0,cent-1))
            last = np.min((n_ch-1, cent+1))
            print(cent, first, last)
...
void main() {
    emitRGB(
        vec3(
            toNormalized(getDataValue("""+str(first)+""")),
            toNormalized(getDataValue("""+str(cent)+""")),
            toNormalized(getDataValue("""+str(last)+""")))
        );
}
""")
#            else:
