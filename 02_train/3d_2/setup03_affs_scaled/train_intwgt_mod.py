...
target_vox = (24,9,9) #(16,4,4) #(36,16,16) #tens of nanometers -- this is the "middle" size of the 3
...
def train(iterations, rej=0.95, save_int=5000):
    save_int = np.min([save_int, iterations])
    ...
    optimizer = torch.optim.Adam(lr=2e-5, 
    ...
    def create_source(sample):
        ...
        source += gp.Reject_CMM(mask=labels, 
                    min_masked=0.005, #0.001 for sc down
                    min_min_masked=0.001, # 0.0002 for sc down 
                    reject_probability=rej) 
    ...
    pipeline += gp.Squeeze([raw, labels, aff_mask, gt, pred, mask_weights])
