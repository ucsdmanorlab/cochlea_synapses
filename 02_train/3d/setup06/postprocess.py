...
#raw_files = sorted(glob.glob('../../../01_data/zarrs/sampled_out/sample_25.zarr')) #hertzano/crops/*.zarr'))
raw_files = sorted(glob.glob('../../../01_data/zarrs/sampled_out/redo_seg/A2_Apex_*.zarr'))
...
def watershed_from_affinities(
    ...
    mean_affs = np.mean(affs[0:2], axis=0) 
    ...
    for z in range(depth):
        ...
        boundary_mask = mean_affs[z]>(thresh*max_affinity_value)
        boundary_distances = distance_transform_edt(boundary_mask)

        if labels_mask is not None:
            boundary_mask *= labels_mask[z].astype(bool)
...
for raw_file in raw_files:
    ...
    in_file = raw_file #raw_file.replace('out', 'images')
    #print(in_file)
    outshape = zarr.open(in_file)['raw'].shape

    merge_thresh = 0.1 #[0.5, 0.5, 0.5]
    affs_thresh = 0.5
    pred = np.asarray(out_file['pred_affs_resize'])
    mask = np.asarray(out_file['pred_lsds_resize'][3])<0.4
    segmentation = get_segmentation(pred, merge_thresh=merge_thresh, affs_thresh=affs_thresh, labels_mask=mask) #next(waterz.agglomerate(affs=pred, thresholds=thresh))
    ...
    out_file['affs_seg'].attrs['offset'] = out_file['pred_affs_resize'].attrs['offset'] 
    out_file['affs_seg'].attrs['resolution'] = out_file['pred_affs_resize'].attrs['resolution']
