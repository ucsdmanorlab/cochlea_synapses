...
data_dir = '../../../01_data/zarrs/selected_training'#ctbp2/restest/training'
...
def train_until(max_iteration):
    ...
    def create_source(sample):
        ...
        #source += rescale_cmm3.ReScale(raw, scalefactor) #scale_augment3.ScaleAugment(raw, scalefactor)
        #source += rescale_cmm3.ReScale(labels, scalefactor, order=0, anti_aliasing=False)
        #source += rescale_cmm3.ReScale(foreground_mask, scalefactor, order=0, anti_aliasing=False)
        ...
        source += rescale_cmm3.ReScale(labels, scalefactor, order=0, anti_aliasing=False)
        source += rescale_cmm3.ReScale(foreground_mask, scalefactor, order=0, anti_aliasing=False)
        source += Reject(mask=foreground_mask, min_masked=0.05, reject_probability=0.95)
