...
if __name__ == '__main__':
    ...
    checkpoint = 'model_checkpoint_320000'
    #raw_files = sorted(glob.glob('../../../01_data/zarrs/nih/ctbp2_restest/*3.zarr'))
    raw_files = sorted(glob.glob('../../../01_data/zarrs/ctbp2/restest/validation/*zarr'))
    #raw_files = sorted(glob.glob('../../../01_data/zarrs/elena-males/*.zarr'))
    ...
    for raw_file in raw_files:
        ...
        raw_type = zarr.open(raw_file)['3d/raw'].dtype
        raw_resize = resize(np.asarray(zarr.open(raw_file)['3d/raw']), 
                outshape, preserve_range=True).astype(raw_type)
        save_out(out_file, raw_resize, 'raw_resize')
        del raw_resize
        ...
        save_out(out_file, np.asarray(pred_affs), 'pred_affs_resize')
        save_out(out_file, np.asarray(pred_lsds), 'pred_lsds_resize')

        #pred_affs = resize(np.asarray(pred_affs), (3,) + inshape, preserve_range=True, channel_axis=0).astype(pred_affs)
        
        #save_out(out_file, zarr.open(raw_file)['3d/raw'][:], 'raw')
        #save_out(out_file, pred_affs, 'pred_affs')
        #save_out(out_file, resize( np.asarray(pred_lsds), (3,) + inshape, preserve_range=True, channel_axis=0).astype(pred_lsds), 'pred_lsds')
