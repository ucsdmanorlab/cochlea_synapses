...
val_dir = '../../../01_data/zarrs/model_2022-10-25_12-46-17_sdt_bgr0_scale3_scaled_checkpoint_10000'
val_samples = [i for i in os.listdir(val_dir) if i.endswith('.zarr')]

thresh_list = [-0.1, 0, 0.1]
max_thresh_list = [0.3, 0.4, 0.5, 0.7, 0.8] 
...
for fi in val_samples:
    ...
    pred = gaussian_filter(pred, sigma=(1,1,1))

    for thresh in thresh_list:
        for max_thresh in max_thresh_list:
            print(thresh, max_thresh)
            pred_mask = pred > thresh

            coords = peak_local_max(pred, 
                    threshold_abs = max_thresh*pred.max(),
                    footprint = np.ones((3,5,5)),
                    labels = pred_mask)
            coord_mask = np.zeros_like(pred, dtype=np.uint16)
            
            for i in range(coords.shape[0]):
                coord_mask[tuple(coords[i].T)] = i
            labels = watershed(-pred, coord_mask, mask=pred_mask)
